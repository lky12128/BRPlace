...
from model_utils import adapt_input_conv, resize_pos_embed, init_weights, padding, unpadding, GeMPooling
from stems import PatchEmbedding, ConvStem
from decoders import DecoderLinear
# DINOv2
from layers import Mlp, NestedTensorBlock as Block
...
class ImageFeatureExtractor(nn.Module):
    ...
    def forward(self, im):
        # 你的融合模型只需要编码器的 tokens，不需要解码器
        # 我们直接调用编码器
        im = padding(im, self.rangevit.patch_size)
        x, _ = self.rangevit.encoder.prepare_tokens(im)

        # x 的形状是 [B, N_tokens + 1 (CLS), C]
        # 这正是你的 fusion_model.py (line 131) 想要的东西
        return x